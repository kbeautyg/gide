...
export interface FilterParams {
  ...
  format?: string[]  // форматы проведения
  transportation?: string[]  // способы передвижения
}
...
export function parseUrlParams(searchParams: URLSearchParams): FilterParams {
  ...
  // Transportation (способы передвижения)
...
export function buildUrlParams(filters: FilterParams): URLSearchParams {
  ...
  if (filters.format && filters.format.length > 0) {
    const uniqueFormats = [...new Set(filters.format.filter(Boolean))]
    if (uniqueFormats.length > 0) {
      params.append('format', uniqueFormats.join(','))
    }
  }

  // Transportation (способы передвижения)
  if (filters.transportation && filters.transportation.length > 0) {
    const uniqueTransportation = [...new Set(filters.transportation.filter(Boolean))]
    if (uniqueTransportation.length > 0) {
      params.append('transportation', uniqueTransportation.join(','))
    }
  }
