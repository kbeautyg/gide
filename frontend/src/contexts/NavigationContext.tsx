...
export function NavigationProvider({ children }: NavigationProviderProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (params.dateStart || params.dateEnd) {
        newState.dateRange = {
          from: params.dateStart ? new Date(params.dateStart) : undefined,
          to: params.dateEnd ? new Date(params.dateEnd) : undefined
        }
      }

      // Обработка format
      if (params.format && params.format.length > 0) {
        newState.format = params.format
      }

      // Обработка transportation
      if (params.transportation && params.transportation.length > 0) {
        newState.transportation = params.transportation
      }
  ...
  const updateUrl = useCallback((newState: NavigationState) => {
    ...
    try {
      ...
      if (newState.dateRange) {
        if (newState.dateRange.from) {
          filters.dateStart = newState.dateRange.from.toISOString().split('T')[0]
        }
        if (newState.dateRange.to) {
          filters.dateEnd = newState.dateRange.to.toISOString().split('T')[0]
        }
      }

      // Format
      if (newState.format.length > 0) {
        filters.format = newState.format
      }

      // Transportation
      if (newState.transportation.length > 0) {
        filters.transportation = newState.transportation
      }
    ...
    setState(prev => {
      ...
      const validCitiesForCountries = newCountries.flatMap(c => {
        ...
        const cityMap: Record<string, string[]> = {
          ...
          'Южная Корея': ['Сеул', 'Пусан', 'Чеджу', 'Инчхон'],
  ...
  const setDateRange = useCallback((dateRange: { from?: Date; to?: Date } | null) => {
    setState(prev => {
  ...
  const setTransportation = useCallback((transportation: string[]) => {
    setState(prev => {
      const newState = { ...prev, transportation }
      updateUrl(newState)
      return newState
    })
  }, [updateUrl])
  ...
  const toggleTransportation = useCallback((transportation: string) => {
    setState(prev => {
      const newState = {
        ...prev,
        transportation: prev.transportation.includes(transportation)
          ? prev.transportation.filter(t => t !== transportation)
          : [...prev.transportation, transportation]
      }
      updateUrl(newState)
      return newState
    })
  ...
  const buildUrl = useCallback(() => {
    ...
    if (state.dateRange) {
      if (state.dateRange.from) {
        filters.dateStart = state.dateRange.from.toISOString().split('T')[0]
      }
      if (state.dateRange.to) {
        filters.dateEnd = state.dateRange.to.toISOString().split('T')[0]
      }
    }

    if (state.format.length > 0) {
      filters.format = state.format
    }

    if (state.transportation.length > 0) {
      filters.transportation = state.transportation
    }
