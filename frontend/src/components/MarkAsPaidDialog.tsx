...
export function MarkAsPaidDialog({ open, onOpenChange, tour }: MarkAsPaidDialogProps) {
  ...
  const markPaidMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Оплата зафиксирована!', `${tour.title} — ${formData.client_name} — ${formatRUB(tour.price * formData.participants_count)}`)
    ...
    onError: (error: any) => {
      toast.error('Ошибка при сохранении оплаты', error.response?.data?.detail)
    },
  })
