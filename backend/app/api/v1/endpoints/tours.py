...
async def get_tours(
    ...
    format: Optional[str] = Query(None, description="Фильтр по форматам проведения (через запятую)"),
    transportation: Optional[str] = Query(None, description="Фильтр по способам передвижения (через запятую)"),
    strict_filter: bool = Query(False, description="Строгий фильтр: AND вместо OR для рубрик и достопримечательностей"),
...
async def get_tour_by_code(
    ...
    # Fallback: if share_code looks like a numeric ID, try looking up by ID
    if not tour_db and share_code.isdigit():
        tour_db = await TourService.get_tour_by_id(db, int(share_code))
...
async def get_dynamic_navigation(
    ...
    Поддерживает фильтры для динамического подсчета счетчиков
    """
    try:
        print("🔍 DEBUG: Starting dynamic navigation generation...")
        navigation_data = await RecommendationService.get_dynamic_categories_from_tours(
            db,
            location=location,
            themes=themes,
            landmarks=landmarks,
...
# ============= ДИНАМИЧЕСКИЕ РОУТЫ (ПОСЛЕ СТАТИЧЕСКИХ!) =============
...
async def reschedule_tour_with_request(
    ...
    # 8. Уведомление клиенту о переносе тура
    try:
        # Ищем бронирования по этому туру, чтобы уведомить клиентов
        bookings_result = await db.execute(
        ...
        notified_users = set()
        for bk in bookings:
            if bk.client_id and bk.client_id not in notified_users:
                await create_notification(
                    db=db,
                    user_id=bk.client_id,
                    title="Тур перенесён",
                    message=f'Экскурсия «{tour.title}» перенесена на {new_date.strftime("%d.%m.%Y")}',
                    type="warning",
                    link="/dashboard/bookings",
                )
                notified_users.add(bk.client_id)
    except Exception as e:
        print(f"⚠️ Не удалось создать уведомление о переносе: {e}")
