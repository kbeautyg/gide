...
async def mark_as_paid(
    ...
    # Уведомление клиенту об оплате (savepoint-safe)
    await create_notification(
        db=db,
        user_id=booking.client_id,
        title="Оплата подтверждена",
        message=f'Экскурсия «{tour.title}» успешно оплачена. Сумма: {total_price}₽',
        type="success",
...
async def get_bookings(
    ...
    """
    Получение списка бронирований текущего гида или клиента
    """
    # Если пользователь - клиент (не гид и не админ), возвращаем только его бронирования
    if current_user.role not in ['admin', 'super_admin', 'manager', 'guide']:
        stmt = (
            select(BookingModel, Tour.title, Tour.photos)
        ...
        bookings_list = [
            {
                ...
                "tour_photo": tour_photos[0] if tour_photos else None,
            ...
            for b, tour_title, tour_photos in bookings_with_tours
        ]
        return BookingList(bookings=bookings_list, total=len(bookings_list))

    # Для админов — все бронирования, для гидов — только их туров
    if current_user.role in ['admin', 'super_admin']:
        ...
        # Гид видит только бронирования своих туров
        tours_result = await db.execute(
            select(Tour.id).where(Tour.guide_id == current_user.id)
        )
        tour_ids = [row[0] for row in tours_result.all()]
...
async def get_system_user_for_anonymous(db: AsyncSession) -> User:
    """Получить системного пользователя для анонимных бронирований (супер-админ)"""
    ...
    if not system_user:
        raise HTTPException(
            status_code=500,
            detail="Системный пользователь не найден. Пожалуйста, выполните инициализацию БД."
        )
...
async def create_booking(
    ...
    # Получаем client_id для бронирования и заявки
    if current_user:
        client_id = current_user.id
    else:
        # Для анонимных пользователей используем системного пользователя
        system_user = await get_system_user_for_anonymous(db)
    ...
    # Уведомление гиду о новом бронировании (savepoint-safe)
    if tour.guide_id:
        await create_notification(
            db=db,
