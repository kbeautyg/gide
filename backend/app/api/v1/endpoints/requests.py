...
async def update_request(
    ...
    # Уведомление клиенту при изменении статуса
    new_status = update_data.get('status')
    if new_status and new_status != old_status and request.client_id:
        status_messages = {
            'in_progress': ('Заявка в работе', f'Заявка «{request.title}» взята в работу.', 'info'),
            'completed': ('Заявка выполнена', f'Заявка «{request.title}» успешно выполнена!', 'success'),
            'cancelled': ('Заявка отклонена', f'Заявка «{request.title}» была отклонена.', 'warning'),
            'assigned': ('Заявка назначена', f'По вашей заявке «{request.title}» назначен гид.', 'success'),
        }
        if new_status in status_messages:
            title, msg, ntype = status_messages[new_status]
            try:
...
async def take_request(
    ...
    # Уведомление клиенту о назначении гида
    try:
        if request.client_id:
            await create_notification(
                ...
                user_id=request.client_id,
                title="Гид назначен",
                message=f'По вашей заявке «{request.title}» назначен гид {current_user.name or ""}. Дата: {assigned_date.strftime("%d.%m.%Y")}',
                type="success",
                link="/dashboard/bookings",
            )
    except Exception as e:
    ...
    try:
        if request.client_id:
            await create_notification(
                ...
                message=f'Заявка «{request.title}» перенесена на {new_date.strftime("%d.%m.%Y")}',
                type="warning",
                link="/dashboard/bookings",
            )
    except Exception as e:
    ...
    try:
        if request.client_id:
            await create_notification(
                ...
                message=f'Заявка «{request.title}» была отменена гидом.',
                type="warning",
                link="/dashboard/bookings",
            )
    except Exception as e:
